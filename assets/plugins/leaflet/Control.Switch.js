...
L.Control.switch = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        var spatialQuery_div = L.DomUtil.create('div', 'spatialQuery_control');
        L.DomEvent.addListener(spatialQuery_div, 'click', L.DomEvent.stopPropagation)
            .addListener(spatialQuery_div, 'click', L.DomEvent.preventDefault)
            .addListener(spatialQuery_div, 'click', function () { onSwith() });
        return spatialQuery_div;
    }
});
...
function onSwith() {
	if(!canClick) {
		...
		$(".spatialQuery_control").css('background-image','url(portal/assets/global/plugins/leaflet/images/switchOn.png)');
		canClick=true;
	}else{
		map.off('click');
		//这里应该是一个事件处理函数，即使是漫游估计也有一些事件需要处理
		$(".spatialQuery_control").css('background-image','url(portal/assets/global/plugins/leaflet/images/switchOff.png)');
	canClick = false;
	}
}
